...
public:
    ...
    /**
     * @brief Parse a GeoJson string returning a geometry pointer with specific type.
     * 
    ...
    std::unique_ptr<T> read(const std::string &geojson_str)
    {
        ...
        if (!g) {
            throw ParseException("Unsupported type for conversion!");
        }
...
protected:
    ...
    std::unique_ptr<geom::GeoGeometry> readGeometryCoordinates(const nlohmann::json &geojson);
...
private:
    ...
    std::unique_ptr<geom::GeoCoordinateSequence> readCoordinateSequence(const nlohmann::json &geojson);

    // GeoJson form: [x,y]
    std::unique_ptr<geom::GeoPoint> readPoint(const nlohmann::json &geojson);
    
    // GeoJson form: [[x1,y1],[x2,y2]]
    std::unique_ptr<geom::GeoLineSegment> readLineSegment(const nlohmann::json &geojson);
    
    // GeoJson form: [[x1,y1],...,[xn,yn]]
    std::unique_ptr<geom::GeoLineString> readLineString(const nlohmann::json &geojson);
    
    // GeoJson form: [[x1,y1],...,[xn,yn],[x1,y1]]
    std::unique_ptr<geom::GeoLinearRing> readLinearRing(const nlohmann::json &geojson);
    ...
    //   ...,
    //   [[xm1,ym1],...,[xmn,ymn]]]
    std::unique_ptr<geom::GeoMultiLineString> readMultiLineString(const nlohmann::json &geojson);
    ...
    std::unique_ptr<geom::GeoMultiPolygon> readMultiPolygon(const nlohmann::json &geojson);
