...
std::string GeoJsonWriter::write(const geom::GeoGeometry *geometry)
{
    ...
    if (geomType == "POINT") {
        ...
        return writeLineSegment(static_cast<const geom::GeoLineSegment *>(geometry));
        ...
        return writeMultiLineString(static_cast<const geom::GeoMultiLineString *>(geometry));
    } else {
        return writeMultiPolygon(static_cast<const geom::GeoMultiPolygon *>(geometry));
    }
}
...
std::string GeoJsonWriter::writeMultiLineString(const geom::GeoMultiLineString *mls)
{
...
nlohmann::json GeoJsonWriter::writeLineSegmentAsJson(const geom::GeoLineSegment *seg)
{
    nlohmann::json json;
    json["type"] = "LineSegment";
    json["coordinates"][0] = writeCoordinateAsJson(seg->getFromPoint()->getCoordinate());
    json["coordinates"][1] = writeCoordinateAsJson(seg->getToPoint()->getCoordinate());
    return json;
}

nlohmann::json GeoJsonWriter::writeLineStringAsJson(const geom::GeoLineString *ls)
{
    nlohmann::json json;
...
nlohmann::json GeoJsonWriter::writeLinearRingAsJson(const geom::GeoLinearRing *lr)
{
    nlohmann::json json;
...
nlohmann::json GeoJsonWriter::writePolygonAsJson(const geom::GeoPolygon *p)
{
    nlohmann::json json;
    json["type"] = "Polygon";
    json["coordinates"][0] = writeLinearRingAsJson(p->getExteriorRing())["coordinates"];
    for (std::size_t i = 0; i != p->getNumInteriorRing(); ++i) {
        json["coordinates"][i + 1] = writeLinearRingAsJson(p->getInteriorRingN(i))["coordinates"];
    }
    return json;
}

nlohmann::json GeoJsonWriter::writeMultiLineStringAsJson(const geom::GeoMultiLineString *mls)
{
    nlohmann::json json;
    json["type"] = "MultiLineString";
    for (std::size_t i = 0; i != mls->getNumLineString(); ++i) {
        json["coordinates"][i] = writeLineStringAsJson(mls->getLineStringN(i))["coordinates"];
    }
    return json;
}

nlohmann::json GeoJsonWriter::writeMultiPolygonAsJson(const geom::GeoMultiPolygon *mp)
{
    nlohmann::json json;
    json["type"] = "MultiPolygon";
    for (std::size_t i = 0; i != mp->getNumPolygon(); ++i) {
        json["coordinates"][i] = writePolygonAsJson(mp->getPolygonN(i))["coordinates"];
    }
    return json;
}

nlohmann::json GeoJsonWriter::writeCoordinateAsJson(const geom::GeoCoordinate *c)
{
    return {c->getLon(), c->getLat()};
}

nlohmann::json GeoJsonWriter::writeCoordinateSequenceAsJson(const geom::GeoCoordinateSequence *cseq)
{
    nlohmann::json json;
